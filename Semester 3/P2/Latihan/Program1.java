...
public class Program1 {
    public static void main(String[] args) {
        ...
        boolean swapped; // swapped untuk pertukaran atau pengurutan nilai variable

        do {
            swapped = false;
            for (int i = 1; i < n; i++) {
                if (nilaiMahasiswa[i - 1] < nilaiMahasiswa[i]){
                    int nilaiTemp = nilaiMahasiswa[i - 1]; // pengurutan nilai array dan menyimpan nilai
        ...
        for (int i = 0; i < banyakMahasiswa; i++) {
            // %s untuk print nilai string 
            // %d untuk print nilai integer
            System.out.printf("%d. Nama = %s, Nilai = %d\n", i + 1, namaMahasiswa[i], nilaiMahasiswa[i]);
        }
        in.close();
    }
}
